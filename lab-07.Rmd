...
```{r viz-score}
ggplot(data = evals, aes(x = cls_perc_eval)) +
  geom_histogram(binwidth = 5) + 
  labs( x = "Age",
        y = NULL)
mean(evals$cls_perc_eval)
median(evals$cls_perc_eval)
```
...
```{r scatterplot}
score_bty_fit <- ggplot(data = evals, aes(x = cls_perc_eval, y = bty_avg)) +
  geom_point() +
...
score_bty_fit <- linear_reg() %>%
  set_engine("lm") %>%
  fit(bty_avg ~ cls_perc_eval, data = evals)
```
